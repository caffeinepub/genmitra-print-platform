...
import {
  Upload, RotateCcw, ZoomIn, ZoomOut, Trash2, Undo2, Redo2,
  Type, Save, ChevronDown, Info, Loader2
} from 'lucide-react';
...
export default function EditorPage() {
  ...
  const pushHistory = useCallback((state: ImageState | null) => {
    setHistory(prev => {
  ...
  const drawCanvas = useCallback(() => {
    ...
    if (imageState) {
      ...
      img.onload = () => {
        ...
        // Calculate dimensions to fit the entire image (object-fit: contain behavior)
  ...
  const handleMouseUp = () => {
    if (isDragging && imageState) {
      pushHistory(imageState);
    }
    setIsDragging(false);
  };
  ...
  const handleZoomIn = () => {
    if (!imageState) return;
    const newState = { ...imageState, scale: Math.min(imageState.scale + 0.1, 3) };
    setImageState(newState);
    pushHistory(newState);
  };
  ...
  const handleZoomOut = () => {
    if (!imageState) return;
    const newState = { ...imageState, scale: Math.max(imageState.scale - 0.1, 0.1) };
    setImageState(newState);
    pushHistory(newState);
  };
  ...
  const handleRotate = () => {
    if (!imageState) return;
    const newState = { ...imageState, rotation: (imageState.rotation + 90) % 360 };
    setImageState(newState);
    pushHistory(newState);
  };
  ...
  const currentSizeLabel = SIZE_OPTIONS.find(s => s.value === selectedSize)?.label ?? selectedSize;
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-3">
          <button
            onClick={() => navigate({ to: '/product/$productId', params: { productId }, search: { category: undefined } })}
          ...
          <div className="flex bg-gray-100 rounded p-0.5">
            <button
              ...
              className={`px-3 py-1.5 text-xs font-medium rounded transition-colors ${
                orientation === 'portrait' ? 'bg-white text-gray-800 shadow-sm' : 'text-gray-500'
              }`}
            >
              Portrait
            </button>
            <button
              ...
              className={`px-3 py-1.5 text-xs font-medium rounded transition-colors ${
                orientation === 'landscape' ? 'bg-white text-gray-800 shadow-sm' : 'text-gray-500'
              }`}
            >
              Landscape
            </button>
      ...
      <div className="flex h-[calc(100vh-64px)]">
          ...
          >
            ...
            {!imageState && (
              <div className="absolute inset-0 flex flex-col items-center justify-center text-gray-400 pointer-events-none">
                ...
                <p className="text-xs mt-1 opacity-70">Upload a photo to get started</p>
              </div>
            )}
          </div>
              ...
              {showSizeDropdown && (
                <div className="absolute top-full left-0 mt-1 bg-white border border-gray-200 rounded shadow-lg z-10 min-w-[200px]">
                  {SIZE_OPTIONS.map(opt => (
                    <button
                      key={opt.value}
                      onClick={() => { setSelectedSize(opt.value); setShowSizeDropdown(false); }}
                      className={`w-full text-left px-4 py-2 text-sm hover:bg-blue-50 transition-colors ${
                        selectedSize === opt.value ? 'text-[#2874f0] font-semibold bg-blue-50' : 'text-gray-700'
                      }`}
                    >
                      {opt.label}
                    </button>
        ...
        <div className="w-64 bg-white border-l border-gray-200 flex flex-col overflow-y-auto">
          <div className="p-4 space-y-4">
            {/* Upload */}
            <div>
              <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Photo</h3>
              <input
                ref={fileInputRef}
                type="file"
                accept="image/*"
            ...
            {/* Frame Style */}
            <div>
              <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Frame Style</h3>
              <FrameStyleDropdown value={frameStyle} onChange={setFrameStyle} />
            </div>

            {/* Adjustments */}
            <div>
              <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Adjustments</h3>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded p-3">
              <div className="flex items-start gap-2">
                <Info className="w-4 h-4 text-[#2874f0] flex-shrink-0 mt-0.5" />
                <div>
                  <p className="text-xs font-semibold text-[#2874f0] mb-1">Pro Tip</p>
                  <p className="text-xs text-blue-700">
                    Upload a high-resolution photo for best print quality. Drag to reposition, use zoom controls to adjust size.
                  </p>
                </div>
              </div>
            </div>
